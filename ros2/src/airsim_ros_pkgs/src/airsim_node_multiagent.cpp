...
int main(int argc, char** argv)
{
    ...
    // Ports 41451 / 41452 / 41453 are fixed by SimModeWorldMultiAgent and are
    // not configurable from the node side — the wrapper handles them internally.

    auto callbackGroup = nh->create_callback_group(rclcpp::CallbackGroupType::Reentrant);

    AirsimROSWrapperMultiAgent airsim_ros_wrapper(
        nh, nh_img, nh_lidar, nh_gpulidar, nh_echo,
        host_ip,
        callbackGroup,
        enable_api_control,
